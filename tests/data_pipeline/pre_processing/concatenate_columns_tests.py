import pandas as pd
import pytest
from trabalho_individual_gces_2022_paulo.data_pipeline.pre_processing.concatenate_columns import (
    get_concatenated_column,
)

# Test cases that should pass
passing_test_cases = [
    {
        "csv": pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6]}),
        "columns": "col1",
        "column_name": "new_col",
        "expected": pd.DataFrame({"new_col": [1, 2, 3]}),
    },
    {
        "csv": pd.DataFrame({"col1": [1, 2, 3], "col2": [4, 5, 6], "col3": [7, 8, 9]}),
        "columns": ["col1", "col2"],
        "column_name": "new_col",
        "expected": pd.DataFrame({"new_col": ["1 4", "2 5", "3 6"]}),
    },
]
